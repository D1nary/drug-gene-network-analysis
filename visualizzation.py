...
def visualize_similarity_subgraph(
    ...
    """Plot a random subgraph of the drug–drug similarity network.

    Parameters
    ----------
    graph : nx.Graph
        Drug similarity network built via ``build_drug_similarity_network``.
    max_nodes : int, optional
        Maximum number of nodes sampled uniformly at random (default 500).
    title : str, optional
        Title for the figure; autogenerated if omitted.
    seed : int, optional
        RNG seed for reproducible sampling/layout. If None, 42 is used so the same
        nodes are chosen on each run.
    community_membership : dict, optional
        Mapping from node name to community label. When provided, nodes are colored
        by community instead of degree.
    ...
    -------
    nx.Graph
        The induced similarity subgraph that was visualized.
    """
    ...
    if title is None:
        title = (
            f"Random Drug Similarity Subgraph "
